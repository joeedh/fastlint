...
async function phaseCompat(exe: string): Promise<void> {
  ...
  for (const method of methods) {
    ...
    for (const key of ["objectId", "type"]) {
      try {
        ...
        // A panic means the handler resolved the id and choked on the type's shape, so the
        // parameter was read under this spelling.
        outcome[key] = msg.includes("empty type handle")
          ? "not read"
          : msg.includes("panic")
            ? "read"
            : msg.slice(0, 48);
  ...
  if (phases.has("--emit")) emitCompatHeader(exe, record);
  await session.release();
  await rpc.close();
}

/**
 * Writes source/fastlint/tsgo/generated/compat.h from the probe results, so the C++ client
 * spells each type-id parameter the way the pinned binary reads it.
 */
function emitCompatHeader(exe: string, record: Record<string, Json>): void {
  const version = execFileSync(exe, ["--version"], { encoding: "utf8" })
    .trim()
    .replace(/^Version\s+/, "");
  const rows: string[] = [];
  for (const [method, outcome] of Object.entries(record)) {
    if (typeof outcome !== "object" || outcome === null || Array.isArray(outcome))
      continue;
    const accepted = Object.entries(outcome as Record<string, string>).find(
      ([, v]) => v === "ok" || v === "read"
    )?.[0];
    if (accepted) rows.push(`    {"${method}", "${accepted}"},`);
  }
  rows.sort();
  const lines = [
    "// Generated by `node tools/spikes/tsgo-api/main.ts compat --emit` against the installed",
    `// tsc ${version}. DO NOT EDIT.`,
    "#pragma once",
    "",
    "namespace fastlint::tsgo::compat {",
    "",
    "/** The tsgo version the parameter table below was probed against. */",
    `constexpr const char *kProbedVersion = "${version}";`,
    "",
    "struct TypeIdParam {",
    "  const char *method;",
    "  const char *param;",
...
const wants = (name: string) =>
  [...phases].filter((p) => !p.startsWith("--")).length === 0 || phases.has(name);
