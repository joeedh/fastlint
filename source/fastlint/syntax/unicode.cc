...
static constexpr CodeRange kIdStart[] = {
...
static constexpr CodeRange kIdContinue[] = {
...
bool contains(const CodeRange *ranges, size_t count, uint32_t cp)
{
  size_t lo = 0;
  size_t hi = count;
  while (lo < hi) {
...
{
  return contains(kIdStart, 685, cp);
}

bool idContinueContains(uint32_t cp)
{
  return contains(kIdContinue, 800, cp);
}
