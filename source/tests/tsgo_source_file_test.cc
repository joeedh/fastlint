 ...
 * layout. */
Vector<uint8_t, 4> encodedFooCall()
{
  ...
  putU32(out, 44);
  putNode(out, SyntaxKind::SourceFile, 0, 7, 0);
  putNode(out, kNodeListKind, 0, 7, 0);
  putNode(out, SyntaxKind::ExpressionStatement, 0, 7, 0);
  putNode(out, SyntaxKind::CallExpression, 0, 6, 2);
  putNode(out, SyntaxKind::Identifier, 0, 3, 3);
  putNode(out, kNodeListKind, 4, 5, 3);
  putNode(out, SyntaxKind::NumericLiteral, 4, 5, 3);
  putNode(out, SyntaxKind::EndOfFile, 7, 7, 0);
  return out;
}
